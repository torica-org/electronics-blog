<p align="center">
  鳥科 電装班の公式ブログへようこそ！
</p>

# 鳥科とは？

東京理科大学 葛飾キャンパスを拠点とする鳥人間サークルです．鳥人間コンテスト 滑空機部門に出場し，私たちで作った飛行機を琵琶湖の空に飛ばすために活動しています．

<span style="font-size:large;"> 詳しくは，[鳥科公式ブログ](https://torica-tus.com)をご確認ください！</span>

# 電装班とは？

機体に搭載する，電子機器の製作を担当している班です．

# 何を作っている？
さて、鳥コンの機体の電子機器が担う役割とは、”機体の操縦”と”フライトログ”の測定です。

”機体の操縦”については、パイロットが操縦桿を握った/動かした分だけ、エレベータ/ラダーのサーボモータを動かして舵角を制御する機構をつくります。

”フライトログの測定”は、以下の”フライトロガー”にあるような項目を計測できる市販のセンサを組み合わせて電子工作します。そして一定間隔でデータを計測し、情報をSDカードに書き込む、通信で同期するという機構をつくるのです。

## フライトロガー

以下のようなデータを取得し，microSDカードに保存するための装置です．
- 対気速度
- 高度（琵琶湖の湖面までの距離）
- 姿勢角
- ...

## 操舵システム

サーボモーターを用いて機体後部の垂直尾翼を駆動しています．
パイロットの手元にあるレバーにより，繊細な操作が可能です．

# 記事

## PR
<!-- 自動で`commercials/`ディレクトリを列挙します -->
{% include list-pages.html dir="commercials/" %}

## 作業
<!-- 自動で`works/`ディレクトリを列挙します -->
{% include list-pages.html dir="works/" %}

## 電装班員の寝言
<!-- 自動で`negoto/`ディレクトリを列挙します -->
{% include list-pages.html dir="negoto/" %}
